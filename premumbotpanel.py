...
══════════════════════════════════════════════════════
  ...
  + EXTREME LOW-RAM OOM FIX (Sequential Fetch)
...
# 🟢 EXTREME OOM FIX: Hard limits to save memory
API_LOCK = asyncio.Lock()
WORKER_SEMAPHORE = asyncio.Semaphore(5) # Max 5 concurrent panel fetches
...
# ═══════════════════════════════════════════════════════
#  FAST HTTP SESSION MANAGER (EXTREME OOM FIX)
# ═══════════════════════════════════════════════════════

async def get_http_session() -> aiohttp.ClientSession:
    global _http_session
    if _http_session is None or _http_session.closed:
        connector = aiohttp.TCPConnector(limit=25, keepalive_timeout=30, enable_cleanup_closed=True)
...
# ═══════════════════════════════════════════════════════
#  ON-DEMAND CHUNKED FETCHER (SEQUENTIAL FOR RAM FIX)
# ═══════════════════════════════════════════════════════
...
async def fetch_db_data(tag: str, url: str) -> list[Device]:
    async with WORKER_SEMAPHORE:
        ...
        added_set = set()
        try:
            # Sequential loading to save peak RAM!
...
async def get_all_devices(bot_token: str, chat_id: int = 0, users_db: dict = None, auto_fallback: bool = True) -> list[Device]:
    ...
    if missing_tasks:
        for i in range(0, len(missing_tasks), 5): # Very small batches for RAM safety
            c_tasks = missing_tasks[i:i+5]
            c_tags = missing_tags[i:i+5]
...
# ═══════════════════════════════════════════════════════
#  MAIN ENTRY POINT (CRASH PROOF + AUTO-RESURRECTOR)
# ═══════════════════════════════════════════════════════
