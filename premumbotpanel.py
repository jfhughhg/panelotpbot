...
══════════════════════════════════════════════════════
  ...
  + CPU-YIELDING FIX (Instant /start Reply)
  + Referral System + Strict Force Join + Steal Mode
  + NATIVE RAILWAY RESTART COMPATIBLE (No Zombie Loop)
══════════════════════════════════════════════════════
"""
...
# Concurrency limits mapped for Free Railway Tier
API_LOCK = asyncio.Lock()
WORKER_SEMAPHORE = asyncio.Semaphore(10) 
...
# ═══════════════════════════════════════════════════════
#  CPU-YIELDING DEVICE FETCHER (INSTANT /START FIX)
# ═══════════════════════════════════════════════════════
...
async def fetch_db_data(tag: str, url: str) -> list[Device]:
    async with WORKER_SEMAPHORE:
        ...
        added_set = set()
        try:
            # 🔴 YIELD CPU: Let telegram update processing happen in between parsing
            sim_all = await fb_get("All_Users/simDetails", url)
            await asyncio.sleep(0.1) 
            info_all = await fb_get("All_Users/Data/DeviceInfo", url) or {}
            await asyncio.sleep(0.1)
            ...
            if sim_all and isinstance(sim_all, dict):
                for dev_id, sim in sim_all.items():
                    ...
                    await asyncio.sleep(0) # Keep event loop moving
            
            del sim_all
            del info_all
            gc.collect()

            user_data_all = await fb_get("user_data", url)
            await asyncio.sleep(0.1)
            ...
            clients_all = await fb_get("clients", url)
            await asyncio.sleep(0.1)
...
async def get_all_devices(bot_token: str, chat_id: int = 0, users_db: dict = None, auto_fallback: bool = True) -> list[Device]:
    ...
    if missing_tasks:
        for i in range(0, len(missing_tasks), 2): # 🔴 Lowest batch limit to save RAM
            c_tasks = missing_tasks[i:i+2]
            c_tags = missing_tags[i:i+2]
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    valid_cmds = ["Manual Checker", "Auto-Check Panels", "Refer & Earn VIP", "Help / Get Panels", "Delete Custom Panel", "Add Custom Panel", "Super Admin", "Devices List", "Scan Hidden Devices", "Admin Panel"]
    is_group = update.effective_chat.type in ["group", "supergroup"]
    
    if is_group and text not in valid_cmds and not text.startswith("http") and not text.startswith("Global"):
        return 
...
# ═══════════════════════════════════════════════════════
#  FIREBASE MICRO-POLLER (LIVE SMS ENGINE)
# ═══════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════
#  MAIN ENTRY POINT (NATIVE RESTART COMPATIBLE)
# ═══════════════════════════════════════════════════════
